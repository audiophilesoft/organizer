 ...
 *
 * Code is optimized by Audiophile
 */
class SortByFieldExtension extends \Twig_Extension
{
     ...
     */
    public function sortByFieldFilter($content, $sort_by = null, $direction = 'asc'): array
    {
        ...
        if (!self::isSortable(current($content), $sort_by)) {
        ...
        @usort($content, function ($a, $b) use ($sort_by, $direction) {
            ...
            if (is_array($a)) {
                ...
                } elseif(method_exists($a, 'is' . ucfirst($sort_by))) {
                    $a_sort_value = $a->{'is' . ucfirst($sort_by)}();
                } else {
                    $a_sort_value = $a->$sort_by;
                }
            }
            if (is_array($b)) {
                ...
                } elseif(method_exists($b, 'is' . ucfirst($sort_by))) {
                    $b_sort_value = $b->{'is' . ucfirst($sort_by)}();
                } else {
                    $b_sort_value = $b->$sort_by;
                }
            }
    ...
    private static function isSortable($item, $field)
    {
        ...
        if (is_object($item)) {
            return isset($item->$field) || property_exists($item, $field) || method_exists($item, 'get' . $field);
        }
